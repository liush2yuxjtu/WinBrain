...
```python
# Rolling windows require chronological row order. Replace `date` with the
# actual time column, or establish a chronological DatetimeIndex first.
...
Use the formulas below only when the prior/beginning value is strictly positive. A zero baseline makes percentage growth undefined; negative baselines require a business-specific alternative such as absolute change, signed percentage-point change, or a separately defined turnaround metric.

```text
Simple growth: (current - previous) / previous
CAGR: (ending / beginning) ^ (1 / years) - 1
Log growth: ln(current / previous)  -- better for volatile positive series
```
