...
<head>
    ...
    <!-- Final dashboards must be fully offline: do not use external script URLs.
         Render charts with native SVG/Canvas, or inline locally vendored Chart.js and
         date-adapter source directly inside script tags during generation. -->
    <style>
...
function filterByDateRange(data, dateField, startDate, endDate) {
    ...
    const endExclusiveKey = endDate ? nextUtcDateKey(endDate) : null;

    return data.filter(row => {
        const rowKey = utcDateKey(row[dateField]);
        if (!rowKey) return false;
        if (startKey && rowKey < startKey) return false;
        if (endExclusiveKey && rowKey >= endExclusiveKey) return false;
        return true;
    });
}
```
...
applyFilters() {
    ...
    const dimensionFiltered = this.rawData.filter(row => {
        if (region && row.region !== region) return false;
        if (category && row.category !== category) return false;
        return true;
    });
    this.filteredData = filterByDateRange(
        dimensionFiltered,
        'date',
        startDate,
        endDate
...
function renderTable(containerId, data, columns) {
    ...
    function render(sortedData) {
        ...
        columns.forEach(col => {
            ...
            button.textContent = `${col.label}${arrow}`;
            button.setAttribute('aria-label', `Sort by ${col.label}`);
            button.addEventListener('click', () => sortBy(col.field));
